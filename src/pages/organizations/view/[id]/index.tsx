...
import { deleteVlogById } from 'apiSdk/vlogs';
...
function OrganizationViewPage() {
  ...
  const vlogHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('vlog', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Vlogs
                    </Text>
                    <NextLink passHref href={`/vlogs/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.vlog?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/vlogs/view/${record.id}`)} key={record.id}>
                            <Td>{record.title}</Td>
